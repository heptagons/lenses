...
We define a relative origin point $p_0$. There are $s$ different points $p_1, p_2, ... p_s$ of magnitud $1$ and equidistant to point $p_0$ located at coordinates $x_i,y_i$ as follows.
\begin{align}
p_i &\equiv \left\{ \begin{array}{ccl}
 \langle x_k,y_k\rangle & \mbox{for} & i \leq t  \quad k = i\\
 \langle x_k, -y_k\rangle & \mbox{for} & i > t \quad k = s+2-i
...
v_i &\equiv \left\{ \begin{array}{ccl}
 ( k, k ) & \mbox{for} & i \leq t \quad k = i\\
 ( k, -k ) & \mbox{for} & i > t \quad k = s+2-i
 \end{array}\right. \label{eq:vectors}
\end{align}

Any vector $v \equiv (k_x,k_y)$ can be rotated by $180^\circ$ around $p_0$ and is denoted as $\overline{v}$ and the pair indices signs are changed:
\begin{align}
\overline{v} &\equiv (-k_x, -k_y) \label{eq:vector180}
\end{align}

We define an absolute origin point $o$ located at $(0,0)$. Starting at $o$ we can form polylines adding vectors alternating the type $v$ and the type $\overline{v}$ in order the consecutive points angles are always multiples of $\theta$. The polyline with vectors $v_a$, $\overline{v_b}$, $v_c$, ... is denoted as:
\begin{align}
...
\\\\
For two consecutive points $P_k = [...,v_m]$ and $P_{k+1} = [...,v_m,\overline{v_n}]$ for $k>0$ we have the angle at point $P_k$ is $u\theta$, where $u$ is:
\begin{align}
