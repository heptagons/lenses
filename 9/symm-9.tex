...
For every symmetry $s$ we identify $t = \dfrac{s+1}2$ independent abscissas $x_i$ and $t$ independent ordinates $y_i$:
\begin{align}
...
The points are transformed into vectors which hold only the $(x,y)$ pair of indices:
\begin{align}
...
Every point $\textbf{P}$ has \texttt{accumulators} composed of two vectors \textbf{X} and \textbf{Y} holding $t$ integers each:
\begin{align}
