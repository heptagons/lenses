...
We define a relative origin point $v_0$. There are $s$ different vectors $v_1, v_2, ... v_s$ of magnitud $1$ and equidistant to point $v_0$ defined with coordinates $x_i,y_i$ as follows.
\begin{align}
...
Any vector $v_m \equiv \langle x_m, y_m\rangle$ where $1 \leq m \leq s$ can be rotated by $180^\circ$ around $v_0$ and is denoted as $\overline{v_m}$ and the pair signs are changed:
\begin{align}
\overline{v_m} &\equiv \langle -x_m, -y_m\rangle \label{eq:vector180}
\end{align}

We define an absolute origin point $o$ located at $(0,0)$. Starting at $o$ we can form polylines adding vectors alternating the type $v_m$ and the type $\overline{v_m}$ in order the consecutive points angles are always multiples of $\theta$. The polyline with vectors $v_a$, $\overline{v_b}$, $v_c$, ... is denoted as:
\begin{align}
\{P_1,P_2,P_3,P_4,...\} &= \{ [], [v_a], [v_a,\overline{v_b}],[v_a,\overline{v_b},v_c],... \} 
 \quad 1 \leq a,b,c \leq t
\end{align}

where point $P_1$ is at origin and the rest of points are array of vectors.
If point $P_k = [...,v_m]$ and $P_{k+1} = [...,v_m,\overline{v_n}]$ for $k>0$ then the angle at point  $P_k$ is $u\theta$, where $u$ is:
\begin{align}
u &\equiv s + m - n \pmod{s} \quad  \quad 1 \leq m,n \leq t
\end{align}
...
\textbf{P}^A &\equiv \left[\begin{array}{c}\textbf{X} \\ \textbf{Y}
 ...
 = \left[\begin{array}{ccccc}X_1&X_2&X_3&...&X_t
  \\ Y_1&Y_2&Y_3&...&Y_t \end{array}\right]
\end{align}

The absolute position of point $P$ can be calculated as follows:
\begin{align}
(P_x,P_y) &= \left( \sum_{i=1}^{t}X_ix_i, \sum_{i=1}^{t}Y_iy_i \right) \label{eq:absolute}
\end{align}
...
Figure \ref{fig:vectors-a} $(ii)$ show polyline $\overline{ABC}$ and $(iii)$ show polyline $\overline{ABCD}$ alternating vectors of the type $[...,v_m]$ (show in red) with vectors of the type $[...,v_m,\overline{v_n}]$ (shown in blue) to guarantee the angles between edges are always multiples of $\delta$: angle $\angle{ABC} = 4\delta$, angle $\angle{BCD} = 2\delta$. New vertices accumulate the preceding vertices by notation. The four vertices $\{A,B,C,D\}$ are denoted as $\{ [\hspace{2pt}], [v_1], [v_1, \overline{v_6}], [v_1,\overline{v_6},v_2] \}$.\
\\
\\
The vertice $o$ at the origin is assigned with accumulators $\textbf{X},\textbf{Y}$ initialized to zero. The next point accumulator increments by $+1$ or $-1$ the position $1 \leq j \leq t$ according the components $x_j, y_j$ the vector $v_m$ has defined. For example we have:

\begin{align}
X[a,b,c,d,e] + v_m &= \left\{ \begin{array}{ccl}
 X[a+1,b,c,d,e] & \mbox{for} & v_1\equiv\langle x_1,y_1\rangle \\
 X[a,b+1,c,d,e] & \mbox{for} & v_2\equiv\langle x_2,y_2\rangle, v_9\equiv\langle x_2,-y_2\rangle\\
 ... & & \\
 X[a,b,c,d,e+1] & \mbox{for} & v_5\equiv\langle x_5,y_5\rangle, v_6\equiv\langle x_5,-y_5\rangle\\
 X[a-1,b,c,d,e] & \mbox{for} & \overline{v_1}\equiv\langle -x_1,-y_1\rangle \\
 ... & &
 \end{array}\right.
\end{align}
